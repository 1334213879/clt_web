<?php /*a:3:{s:74:"D:\phpStudy\PHPTutorial\WWW\clt_web\application\user\view\index\index.html";i:1573700289;s:79:"D:\phpStudy\PHPTutorial\WWW\clt_web\application\user\view\common\loginHead.html";i:1573700711;s:76:"D:\phpStudy\PHPTutorial\WWW\clt_web\application\user\view\common\footer.html";i:1573610849;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title><?php echo htmlentities($title); ?></title>
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
    <meta name="keywords" content="<?php echo htmlentities($title); ?>">
    <meta name="description" content="<?php echo htmlentities($title); ?>">
    <link rel="stylesheet" href="/static/plugins/layui/css/layui.css">
    <link rel="stylesheet" href="/static/user/css/global.css">
</head>
<body>
<div class="header">
    <div class="main">
        <a class="logo" href="/" title="淘钉智能财税">淘钉智能财税</a>
        <div class="nav-user">
            <a class="avatar" href="<?php echo url('index/index'); ?>">
                <?php if($userInfo['avatar']): ?>
                <img src="<?php echo imgUrl($userInfo['avatar']); ?>" alt="<?php echo htmlentities($userInfo['username']); ?>">
                <?php else: ?>
                <img src="/static/user/images/avatar/default.png" alt="<?php echo htmlentities($userInfo['username']); ?>">
                <?php endif; ?>
                <cite><?php echo htmlentities($userInfo['username']); ?></cite>
                <i><?php echo htmlentities($userInfo['level_name']); ?></i>
            </a>
            <div class="nav">
                <a href="<?php echo url('set/index'); ?>"><i class="iconfont icon-shezhi"></i>设置</a>
                <a href="<?php echo url('index/logout'); ?>"><i class="iconfont icon-tuichu" style="top: 0; font-size: 22px;"></i>退了</a>
            </div>

        </div>
    </div>
</div>
<div class="fly-home">
    <?php if($userInfo['avatar']): ?>
    <img src="<?php echo imgUrl($userInfo['avatar']); ?>" alt="<?php echo htmlentities($userInfo['username']); ?>">
    <?php else: ?>
    <img src="/static/user/images/avatar/default.png" alt="<?php echo htmlentities($userInfo['username']); ?>">
    <?php endif; ?>
    <h1>
        <?php echo htmlentities($userInfo['username']); if($userInfo['sex'] == 1): ?>
        <i class="iconfont icon-nan"></i>
        <?php else: ?>
        <i class="iconfont icon-nv"></i>
        <?php endif; ?>
    </h1>
    <p class="fly-home-info">
        <i class="iconfont icon-shijian"></i><span><?php echo toDate($userInfo['reg_time'],'Y-m-d'); ?> 加入</span>
        <?php if($userInfo['city']): ?>
        <i class="iconfont icon-chengshi"></i><span><?php echo toCity($userInfo['city']); ?></span>
        <?php endif; ?>
    </p>
    <?php if(isset($userInfo['sign'])): ?>
    <p class="fly-home-sign">（ <?php echo htmlentities($userInfo['sign']); ?>）</p>
    <?php endif; ?>
</div>


<div class="footer">
    <p><a href="http://www.cltphp.com/">CLTPHP</a> 2017 &copy; <a href="http://www.cltphp.com/">cltphp.com</a></p>
    <p>
        <a href="<?php echo url('home/services/index',array('catId'=>3)); ?>" target="_blank">产品服务</a>
        <a href="<?php echo url('home/system/index',array('catId'=>4)); ?>" target="_blank">系统操作</a>
        <a href="<?php echo url('home/news/index',array('catId'=>5)); ?>" target="_blank">CLTPHP动态</a>
    </p>
</div>
<script src="/static/plugins/layui/layui.js"></script>



</body>
</html>